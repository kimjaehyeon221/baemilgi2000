...
export default function App() {
  ...
  if (challengeLevel !== null) {
    return <Challenge level={challengeLevel} onCancel={() => setChallengeLevel(null)} onFinish={async (success, seconds) => {
      ...
      const saved = await commit({ ...state, clearedLevel, selectedLevel: success ? Math.min(200, Math.max(clearedLevel + 1, state.selectedLevel)) : state.selectedLevel, sessions: [...state.sessions, { at: new Date().toISOString(), type: 'challenge', level: challengeLevel, target: targetForLevel(challengeLevel), success, seconds }] });
  ...
  if (trainingLevel !== null) {
    ...
    return <Training level={trainingLevel} currentBest={currentBest} onCancel={() => setTrainingLevel(null)} onFinish={async (seconds) => {
      const saved = await commit({ ...state, sessions: [...state.sessions, { at: new Date().toISOString(), type: 'training', level: trainingLevel, target: targetForLevel(trainingLevel), success: true, seconds }] });
  ...
  const exportRecords = async () => {
    const payload = {
      ...
      progress: {
        firstBaemilgiMax: state.firstBaemilgiMax,
        clearedLevel: state.clearedLevel,
        currentReps,
    ...
    try {
      await Share.share({ title: '배밀기 2000 기록', message: JSON.stringify(payload, null, 2) });
    } catch {
      Alert.alert('내보내기 실패', '잠시 후 다시 시도해줘.');
    }
  };

  const reset = () => Alert.alert('기록을 모두 지울까?', '이 기기에 저장된 진행 기록이 모두 삭제돼. 먼저 기록을 내보낼 수 있어.', [
    ...
    <SafeAreaView style={styles.root}>
      ...
      {tab === 'records' && (
        <ScrollView contentContainerStyle={styles.page}>
          ...
          <Text style={styles.pageCopy}>앱 사용 시간이 아니라 실제 기록이 얼마나 늘었는지가 중요해. 전체 {state.sessions.length}회의 세션을 이 기기에 보관 중이야.</Text>
          ...
          <View style={{ height: 22 }} /><Button label="전체 기록 내보내기" secondary onPress={exportRecords} />
        </ScrollView>
      )}
